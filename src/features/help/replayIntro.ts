 ...
 * for an anonymous session (`app/index.tsx`, `status === "anon"`), and the
 * tour's own exit (`app/(auth)/tanitim.tsx`'s `finish()`) re-sets the flag
 * via `markIntroSeen()` before leaving. The caller here is always authed,
 * so the flag this clears is not what puts the tour on screen — navigating
 * to `/(auth)/tanitim` directly is. Kept anyway because the brief asks for
 * it and it is not actively wrong: the one case it changes anything is a
 * user who abandons the replayed tour mid-way, later signs out, and
 * cold-starts — they would see the tour again on that next launch.
 */
